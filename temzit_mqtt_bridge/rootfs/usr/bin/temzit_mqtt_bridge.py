#!/usr/bin/env python3
"""
Temzit MQTT Bridge v0.7.3

- На базе 0.7.1: корректное чтение SYNC/CFG по HM_Protocol
- ДОБАВЛЕНА безопасная запись: меняем только cfg_dhw_target (уставка ГВС)
  через MQTT-топик temzit/climate/set_dhw_temp
- Остальные команды записи по-прежнему игнорируются
"""
import os
import time
import json
import socket
import threading

import paho.mqtt.client as mqtt

TEMZIT_HOST                 = os.getenv('TEMZIT_HOST', '192.168.2.20')
TEMZIT_PORT                 = int(os.getenv('TEMZIT_PORT', '333'))
TEMZIT_TIMEOUT              = int(os.getenv('TEMZIT_TIMEOUT', '15'))
TEMZIT_SYNC_INTERVAL        = int(os.getenv('TEMZIT_SYNC_INTERVAL', '60'))
TEMZIT_CFG_INTERVAL         = int(os.getenv('TEMZIT_CFG_INTERVAL', '900'))
TEMZIT_CFG_DELAY_AFTER_SYNC = int(os.getenv('TEMZIT_CFG_DELAY_AFTER_SYNC', '12'))
TEMZIT_RETRY_DELAY          = int(os.getenv('TEMZIT_RETRY_DELAY', '15'))
MQTT_HOST                   = os.getenv('MQTT_HOST', '192.168.1.50')
MQTT_PORT                   = int(os.getenv('MQTT_PORT', '1883'))
MQTT_USER                   = os.getenv('MQTT_USER', '')
MQTT_PASS                   = os.getenv('MQTT_PASS', '')
MQTT_PREFIX                 = os.getenv('MQTT_PREFIX', 'temzit')
MQTT_DISCOVERY_PREFIX       = os.getenv('MQTT_DISCOVERY_PREFIX', 'homeassistant')
MQTT_CLIENT_ID              = os.getenv('MQTT_CLIENT_ID', 'temzit-bridge')

VERSION = "0.7.2-dhw-write"

CMD_SYNC    = 0x30
CMD_REQCFG  = 0x34
CMD_SETCFG  = 0x35
RESP_ACTUAL = 0x01
RESP_CONFIG = 0x02

MODE_CODE_TO_HA = {
    0: 'off',
    1: 'heat',
    2: 'heat',
    3: 'heat',
    4: 'cool',
    5: 'heat',
}
HA_MODES = ['off', 'heat', 'cool']

P1_NAMES = {
    0: 'Стоп',
    1: 'Нагрев',
    2: 'Быстрый',
    3: 'ТЭН',
    4: 'Холод',
    5: 'Внешний',
}

DHW_MODE_NAMES = {
    0: 'Выключен',
    1: 'Только ТЭН в баке',
    2: 'ТН 10%',
    3: 'ТН 20%',
    4: 'ТН 30%',
    5: 'ТН 40%',
    6: 'ТН 50%',
    7: 'ТН 60%',
    8: 'ТН 70%',
    9: 'ТН 80%',
    10: 'ТН 90%',
    11: 'ТН 100%',
}

TEN_MODE_NAMES = {
    0: 'Выключен',
    1: 'Использовать до 20%',
    2: 'Использовать до 40%',
    3: 'Использовать до 60%',
    4: 'Использовать до 80%',
    5: 'Использовать до 100%',
}

COMP_LIMIT_NAMES = {
    0: 'Без ограничений',
    1: '10%',
    2: '20%',
    3: '30%',
    4: '40%',
    5: '50%',
    6: '55%',
    7: '60%',
    8: '70%',
    9: '80%',
    10: '90%',
}

COMP_LIMIT_PCT = {
    0: 0,
    1: 10,
    2: 20,
    3: 30,
    4: 40,
    5: 50,
    6: 55,
    7: 60,
    8: 70,
    9: 80,
    10: 90,
}

BACKUP_TYPE_NAMES = {
    0: 'Не использовать',
    1: 'после I ступени',
    2: 'после II ступени',
    3: 'после III ступени',
    4: 'только внешний',
}

PUMP_RELAY_NAMES = {
    0: 'отопление',
    1: 'отопление и ГВС',
    2: 'всегда включен',
}

FLOWMETER_TYPES = {
    0: 'unknown',
    1: 'impulse_1l',
    2: 'impulse_10l',
    3: 'dual_channel',
    4: 'electronic',
    5: 'fixed',
    6: 'reed_switch',
}

ALARM_BITS = {
    0x0001: 'contactor_fail_e05',
    0x0002: 'link_fail_e08',
    0x0004: 'flow1_fail_e01',
    0x0008: 'wifiterm_fail_e07',
    0x0010: 'tevap_fail_e09',
    0x0020: 'tcompover_fail_e02',
    0x0040: 'tevaplow_fail_e03',
    0x0080: 'kkb1_fail_e04',
    0x0100: 'clock_fail_e06',
    0x0200: 'wifi_error_e0A',
    0x4000: 'crit_tsens_fail',
    0x8000: 'lcdversion_fail',
}


def s8(v: int | None) -> int | None:
    if v is None:
        return None
    return v if v < 128 else v - 256


def weather_comp_from_raw(v: int | None) -> float | None:
    if v is None:
        return None
    return round(v * 0.1, 1)


def checksum16(data: bytes) -> int:
    return sum(data) & 0xFFFF


def u16le(buf: bytes, off: int) -> int | None:
    if len(buf) < off + 2:
        return None
    return int.from_bytes(buf[off:off+2], 'little', signed=False)


def heater_stage_from_raw(v: int | None) -> int | None:
    if v is None:
        return None
    if 9 <= v <= 84:
        return 1
    if 85 <= v <= 169:
        return 2
    if 170 <= v <= 255:
        return 3
    return 0


def dhw_heater_on(v: int | None) -> str | None:
    if v is None:
        return None
    return 'ON' if (v & 0x1) else 'OFF'


def decode_alarm(v: int | None) -> str | None:
    if v is None:
        return None
    names = [name for bit, name in ALARM_BITS.items() if v & bit]
    return 'ok' if not names else ','.join(names)


class TemzitClient:
    def __init__(self, host: str, port: int, timeout: int) -> None:
        self.host = host
        self.port = port
        self.timeout = timeout
        self.lock = threading.Lock()

    def _query(self, payload: bytes) -> tuple[bytes, int]:
        with self.lock:
            t0 = time.time()
            with socket.create_connection((self.host, self.port), timeout=self.timeout) as s:
                s.settimeout(self.timeout)
                s.sendall(payload)
                data = s.recv(256)
            dt = round((time.time() - t0) * 1000)
            return data, dt

    def get_sync(self) -> dict:
        data, dt = self._query(bytes([CMD_SYNC, 0x00]))
        if len(data) < 64:
            raise ValueError(f'incomplete sync reply: {len(data)} bytes')
        if data[0] != RESP_ACTUAL:
            raise ValueError(f'unexpected sync reply type: {data[0]}')
        crc_rx = int.from_bytes(data[62:64], 'little')
        crc_calc = checksum16(data[:62])
        if crc_rx != crc_calc:
            raise ValueError(f'sync CRC mismatch: rx={crc_rx} calc={crc_calc}')

        p = data[2:62]

        def t(off: int) -> float | None:
            v = u16le(p, off)
            return None if v is None else v / 10.0

        flow_raw = u16le(p, 18)
        heater_state = u16le(p, 24)
        dhw_state = u16le(p, 26)
        alarm = u16le(p, 30)
        mode_code = u16le(p, 0)
        power_raw = u16le(p, 28)

        compressor_hz_1 = p[22] if len(p) > 22 else None
        compressor_hz_2 = p[23] if len(p) > 23 else None
        compressor_active = 'ON' if (compressor_hz_1 or 0) > 0 or (compressor_hz_2 or 0) > 0 else 'OFF'

        set_room = p[49] if len(p) > 49 else None
        set_water = p[50] if len(p) > 50 else None
        set_dhw = p[51] if len(p) > 51 else None
        set_comp_limit_raw = p[52] if len(p) > 52 else None
        set_ten_mode_raw = p[53] if len(p) > 53 else None
        set_dhw_mode_raw = p[54] if len(p) > 54 else None

        return {
            'diag_sync_len': len(data),
            'diag_sync_ms': dt,
            '_sync_raw': list(p),
            'mode_code': mode_code,
            'mode_name': P1_NAMES.get(mode_code, f'mode_{mode_code}'),
            'schedule_no': u16le(p, 2),
            't_outdoor': t(4),
            't_room': t(6),
            't_supply': t(8),
            't_return': t(10),
            't_freon_gas': t(12),
            't_freon_liquid': t(14),
            't_dhw': t(16),
            'flow_raw': flow_raw,
            'flow_l_min': None if flow_raw is None else round(flow_raw * 4, 2),
            'compressor_hz_1': compressor_hz_1,
            'compressor_hz_2': compressor_hz_2,
            'compressor_active': compressor_active,
            'heater_state_raw': heater_state,
            'heater_stage': heater_stage_from_raw(heater_state),
            'dhw_heater_state_raw': dhw_state,
            'dhw_heater_on': dhw_heater_on(dhw_state),
            'power_kw': None if power_raw is None else round(power_raw / 10.0, 1),
            'alarm': alarm,
            'alarm_text': decode_alarm(alarm),
            'fw_version_major': p[43] if len(p) > 43 else None,
            'fw_version_minor': p[44] if len(p) > 44 else None,
            'active_schedule_no': p[45] if len(p) > 45 else None,
            'active_schedule_mode': p[46] if len(p) > 46 else None,
            'set_room': set_room,
            'set_water': set_water,
            'set_dhw': set_dhw,
            'set_compressor_limit': set_comp_limit_raw,
            'set_compressor_limit_pct': COMP_LIMIT_PCT.get(set_comp_limit_raw),
            'set_compressor_limit_name': COMP_LIMIT_NAMES.get(set_comp_limit_raw, str(set_comp_limit_raw)),
            'set_ten_mode': set_ten_mode_raw,
            'set_ten_mode_name': TEN_MODE_NAMES.get(set_ten_mode_raw, str(set_ten_mode_raw)),
            'set_dhw_mode': set_dhw_mode_raw,
            'set_dhw_mode_name': DHW_MODE_NAMES.get(set_dhw_mode_raw, str(set_dhw_mode_raw)),
            'weekday': p[56] if len(p) > 56 else None,
            'hour': p[57] if len(p) > 57 else None,
            'minute': p[58] if len(p) > 58 else None,
            'second': p[59] if len(p) > 59 else None,
        }

    def get_cfg(self) -> dict:
        data, dt = self._query(bytes([CMD_REQCFG, 0x00]))
        if len(data) < 64:
            raise ValueError(f'incomplete cfg reply: {len(data)} bytes')
        if data[0] != RESP_CONFIG:
            raise ValueError(f'unexpected cfg reply type: {data[0]}')
        crc_rx = int.from_bytes(data[62:64], 'little')
        crc_calc = checksum16(data[:62])
        if crc_rx != crc_calc:
            raise ValueError(f'cfg CRC mismatch: rx={crc_rx} calc={crc_calc}')

        # CONFIG_MAIN: 30 байт начиная с offset 1
        p = data[1:31]

        ten_byte = p[3] if len(p) > 3 else None
        dhw_byte = p[6] if len(p) > 6 else None
        solar_byte = p[23] if len(p) > 23 else None

        cfg_home_inertia = (ten_byte >> 4) & 0x0F if ten_byte is not None else None
        cfg_ten_mode = ten_byte & 0x0F if ten_byte is not None else None
        cfg_disinfection = (dhw_byte >> 4) & 0x0F if dhw_byte is not None else None
        cfg_dhw_mode = dhw_byte & 0x0F if dhw_byte is not None else None
        cfg_solar_overheat_action = solar_byte & 0x1F if solar_byte is not None else None
        cfg_solar_mode = (solar_byte >> 5) & 0x07 if solar_byte is not None else None

        comp_limit_raw = p[9] if len(p) > 9 else None
        weather_raw = p[18] if len(p) > 18 else None
        pump_relay_raw = p[20] if len(p) > 20 else None
        backup_raw = p[8] if len(p) > 8 else None
        flowmeter_type = p[22] if len(p) > 22 else None

        return {
            'diag_cfg_len': len(data),
            'diag_cfg_ms': dt,
            'cfg_mode': p[0] if len(p) > 0 else None,
            'cfg_room_target': p[1] if len(p) > 1 else None,
            'cfg_water_target': p[2] if len(p) > 2 else None,
            'cfg_home_inertia': cfg_home_inertia,
            'cfg_ten_mode': cfg_ten_mode,
            'cfg_ten_mode_name': TEN_MODE_NAMES.get(cfg_ten_mode, str(cfg_ten_mode)),
            'cfg_ten_on_outdoor': s8(p[4] if len(p) > 4 else None),
            'cfg_kkb_off_outdoor': s8(p[5] if len(p) > 5 else None),
            'cfg_disinfection': cfg_disinfection,
            'cfg_dhw_mode': cfg_dhw_mode,
            'cfg_dhw_mode_name': DHW_MODE_NAMES.get(cfg_dhw_mode, str(cfg_dhw_mode)),
            'cfg_dhw_target': p[7] if len(p) > 7 else None,
            'cfg_backup_type': backup_raw,
            'cfg_backup_type_name': BACKUP_TYPE_NAMES.get(backup_raw, str(backup_raw)),
            'cfg_compressor_limit': comp_limit_raw,
            'cfg_compressor_limit_pct': COMP_LIMIT_PCT.get(comp_limit_raw),
            'cfg_compressor_limit_name': COMP_LIMIT_NAMES.get(comp_limit_raw, str(comp_limit_raw)),
            'cfg_meter_impulses': p[17] if len(p) > 17 else None,
            'cfg_weather_comp': weather_comp_from_raw(weather_raw),
            'cfg_collector_off': (p[19] & 0x0F) if len(p) > 19 else None,
            'cfg_collector_on': ((p[19] >> 4) & 0x0F) if len(p) > 19 else None,
            'cfg_pump_relay_mode': pump_relay_raw,
            'cfg_pump_relay_mode_name': PUMP_RELAY_NAMES.get(pump_relay_raw, str(pump_relay_raw)),
            'cfg_dhw_max_from_compressor': p[21] if len(p) > 21 else None,
            'cfg_flowmeter_type': flowmeter_type,
            'cfg_flowmeter_type_name': FLOWMETER_TYPES.get(flowmeter_type, f'unknown_{flowmeter_type}'),
            'cfg_solar_overheat_action': cfg_solar_overheat_action,
            'cfg_solar_mode': cfg_solar_mode,
            'cfg_solar_overheat_temp': p[24] if len(p) > 24 else None,
            'cfg_comp1_type': p[25] if len(p) > 25 else None,
            '_raw': list(p),
        }

    def set_cfg(self, cfg_bytes: list[int]) -> None:
        """Отправка CONFIG_MAIN (30 байт) командой 0x35."""
        if len(cfg_bytes) != 30:
            raise ValueError(f'CONFIG_MAIN must be 30 bytes, got {len(cfg_bytes)}')
        body = bytes(cfg_bytes)
        cmd = bytes([CMD_SETCFG])
        without_kc = cmd + body
        kc = (sum(without_kc)) & 0xFF
        with_kc = without_kc + bytes([kc])
        ks = (sum(with_kc)) & 0xFF
        packet = with_kc + bytes([ks])
        self._query(packet)


class Bridge:
    def __init__(self) -> None:
        self.temzit = TemzitClient(TEMZIT_HOST, TEMZIT_PORT, TEMZIT_TIMEOUT)
        self.client = mqtt.Client(client_id=MQTT_CLIENT_ID, clean_session=True)
        if MQTT_USER:
            self.client.username_pw_set(MQTT_USER, MQTT_PASS)
        self.client.on_connect = self.on_connect
        self.client.on_message = self.on_message
        self.discovery_sent = False
        self.last_cfg_poll = 0.0
        self.last_sync_ts = 0.0
        self._last_cfg_raw: list[int] | None = None

    def publish(self, topic: str, payload, retain: bool = True, qos: int = 0) -> None:
        if not isinstance(payload, str):
            payload = json.dumps(payload, ensure_ascii=False)
        self.client.publish(topic, payload, qos=qos, retain=retain)

    def on_connect(self, client, userdata, flags, rc):  # type: ignore[override]
        self.publish(f'{MQTT_PREFIX}/availability', 'online')
        client.subscribe(f'{MQTT_PREFIX}/climate/set_mode')
        client.subscribe(f'{MQTT_PREFIX}/climate/set_temperature')
        client.subscribe(f'{MQTT_PREFIX}/climate/set_water_temp')
        client.subscribe(f'{MQTT_PREFIX}/climate/set_dhw_temp')
        client.subscribe(f'{MQTT_PREFIX}/climate/set_compressor_limit')
        client.subscribe(f'{MQTT_PREFIX}/cmd/set_byte')

    def _handle_set_dhw(self, payload: str) -> None:
        try:
            temp = float(payload)
        except ValueError:
            self.publish(f'{MQTT_PREFIX}/bridge/error', {
                'set_dhw_error': 'invalid_payload',
                'payload': payload,
            })
            return

        if self._last_cfg_raw is None or len(self._last_cfg_raw) != 30:
            self.publish(f'{MQTT_PREFIX}/bridge/error', {
                'set_dhw_error': 'no_cfg_cache',
                'payload': payload,
            })
            return

        value = int(round(temp))
        value = max(20, min(70, value))

        cfg_before = list(self._last_cfg_raw)
        cfg = list(cfg_before)
        cfg[7] = value

        try:
            self.publish(f'{MQTT_PREFIX}/bridge/info', {
                'set_dhw_requested': value,
                'cfg_before': cfg_before,
                'cfg_after_local': cfg,
            })

            # 1. Запись CONFIG_MAIN
            self.temzit.set_cfg(cfg)
            self._last_cfg_raw = cfg

            self.publish(f'{MQTT_PREFIX}/bridge/info', {
                'set_dhw_applied_cfg': value,
            })

            # Пауза после записи, чтобы контроллер применил конфиг
            time.sleep(3.0)

            # 2. Отдельно читаем свежий CFG
            try:
                fresh_cfg = self.temzit.get_cfg()
                self._publish_cfg(fresh_cfg)
                self.publish(f'{MQTT_PREFIX}/bridge/info', {
                    'set_dhw_cfg_readback_ok': True,
                    'cfg_readback_raw': fresh_cfg.get('_raw'),
                    'cfg_readback_target': fresh_cfg.get('cfg_dhw_target'),
                })
            except Exception as ce:
                self.publish(f'{MQTT_PREFIX}/bridge/error', {
                    'set_dhw_error': 'cfg_readback_failed',
                    'detail': str(ce),
                })
                return

            # Дополнительная пауза между get_cfg и get_sync
            time.sleep(3.0)

            # 3. Потом отдельно читаем SYNC
            try:
                fresh_state = self.temzit.get_sync()
                self._publish_state(fresh_state)
                self.publish(f'{MQTT_PREFIX}/bridge/info', {
                    'set_dhw_sync_readback_ok': True,
                    'sync_set_dhw': fresh_state.get('set_dhw'),
                    'sync_t_dhw': fresh_state.get('t_dhw'),
                })
            except Exception as se:
                self.publish(f'{MQTT_PREFIX}/bridge/error', {
                    'set_dhw_error': 'sync_readback_failed',
                    'detail': str(se),
                })

        except Exception as e:
            self.publish(f'{MQTT_PREFIX}/bridge/error', {
                'set_dhw_error': 'transport',
                'detail': str(e),
            })

    def on_message(self, client, userdata, msg):  # type: ignore[override]
        topic = msg.topic
        payload = msg.payload.decode('utf-8', errors='ignore').strip()

        if topic == f'{MQTT_PREFIX}/climate/set_dhw_temp':
            self._handle_set_dhw(payload)
            return

        # Остальное пока игнорируем
        self.publish(f'{MQTT_PREFIX}/bridge/error', {
            'readonly_mode': True,
            'ignored_topic': topic,
            'ignored_payload': payload,
        })

    def publish_discovery(self) -> None:
        if self.discovery_sent:
            return
        device = {
            'identifiers': ['temzit_hp_1'],
            'name': 'Temzit Heat Pump',
            'manufacturer': 'ТЭМЗИТ',
            'model': 'Hydromodule',
            'sw_version': VERSION,
        }
        avail = {
            'availability_topic': f'{MQTT_PREFIX}/availability',
            'payload_available': 'online',
            'payload_not_available': 'offline',
        }

        # Основной climate (отопление)
        self.publish(f'{MQTT_DISCOVERY_PREFIX}/climate/temzit_climate/config', {
            'name': 'Temzit',
            'uniq_id': 'temzit_climate',
            'device': device,
            **avail,
            'curr_temp_t': f'{MQTT_PREFIX}/state/t_room',
            'temp_stat_t': f'{MQTT_PREFIX}/state/climate_target_temp',
            'temp_cmd_t': f'{MQTT_PREFIX}/climate/set_temperature',
            'temp_step': 1,
            'min_temp': 16,
            'max_temp': 30,
            'mode_stat_t': f'{MQTT_PREFIX}/state/ha_mode',
            'mode_cmd_t': f'{MQTT_PREFIX}/climate/set_mode',
            'modes': HA_MODES,
            'precision': 0.1,
        })

        # ГВС как отдельный climate
        self.publish(f'{MQTT_DISCOVERY_PREFIX}/climate/temzit_dhw_climate/config', {
            'name': 'Temzit ГВС',
            'uniq_id': 'temzit_dhw_climate',
            'device': device,
            **avail,
            'curr_temp_t': f'{MQTT_PREFIX}/state/t_dhw',
            'temp_stat_t': f'{MQTT_PREFIX}/state/set_dhw',
            'temp_cmd_t': f'{MQTT_PREFIX}/climate/set_dhw_temp',
            'temp_step': 1,
            'min_temp': 20,
            'max_temp': 70,
            'mode_stat_t': f'{MQTT_PREFIX}/state/dhw_ha_mode',
            'mode_cmd_t': f'{MQTT_PREFIX}/climate/set_mode',
            'modes': ['off', 'heat'],
            'precision': 1,
        })

        sensors = [
            ('outdoor_temperature', 'Температура улица', 't_outdoor', 'temperature', '°C'),
            ('room_temperature', 'Температура дом', 't_room', 'temperature', '°C'),
            ('supply_temperature', 'Температура подачи', 't_supply', 'temperature', '°C'),
            ('return_temperature', 'Температура обратки', 't_return', 'temperature', '°C'),
            ('dhw_temperature', 'Температура ГВС', 't_dhw', 'temperature', '°C'),
            ('freon_gas_temperature', 'Фреон газ', 't_freon_gas', 'temperature', '°C'),
            ('freon_liquid_temperature', 'Фреон жидкость', 't_freon_liquid', 'temperature', '°C'),
            ('power_kw', 'Мощность', 'power_kw', 'power', 'kW'),
            ('flow_l_min', 'Проток', 'flow_l_min', None, 'L/min'),
            ('compressor_hz_1', 'ККБ1 частота', 'compressor_hz_1', 'frequency', 'Hz'),
            ('compressor_hz_2', 'ККБ2 частота', 'compressor_hz_2', 'frequency', 'Hz'),
            ('heater_stage', 'Ступень ТЭНа', 'heater_stage', None, None),
            ('mode_name', 'Режим работы', 'mode_name', None, None),
            ('set_room', 'Уставка t помещения (активная)', 'set_room', 'temperature', '°C'),
            ('set_water', 'Уставка t воды (активная)', 'set_water', 'temperature', '°C'),
            ('set_dhw', 'Уставка t ГВС (активная)', 'set_dhw', 'temperature', '°C'),
            ('set_ten_mode_name', 'Режим ТЭНа (активный)', 'set_ten_mode_name', None, None),
            ('set_dhw_mode_name', 'Режим ГВС (активный)', 'set_dhw_mode_name', None, None),
            ('set_compressor_limit_name', 'Лимит ККБ (активный)', 'set_compressor_limit_name', None, None),
            ('cfg_room_target', 'Конфиг t помещения', 'cfg_room_target', 'temperature', '°C'),
            ('cfg_water_target', 'Конфиг t воды', 'cfg_water_target', 'temperature', '°C'),
            ('cfg_dhw_target', 'Конфиг t ГВС', 'cfg_dhw_target', 'temperature', '°C'),
            ('cfg_dhw_mode_name', 'Режим ГВС (конфиг)', 'cfg_dhw_mode_name', None, None),
            ('cfg_ten_mode_name', 'Режим ТЭНа (конфиг)', 'cfg_ten_mode_name', None, None),
            ('cfg_compressor_limit_name', 'Лимит ККБ (конфиг)', 'cfg_compressor_limit_name', None, None),
            ('cfg_weather_comp', 'Погодокомпенсация', 'cfg_weather_comp', None, None),
            ('cfg_home_inertia', 'Коэффициент инерции дома', 'cfg_home_inertia', None, None),
            ('cfg_ten_on_outdoor', 'Температура включения ТЭНа', 'cfg_ten_on_outdoor', 'temperature', '°C'),
            ('cfg_kkb_off_outdoor', 'Температура выключения компрессора', 'cfg_kkb_off_outdoor', 'temperature', '°C'),
            ('cfg_dhw_max_from_compressor', 'Макс t ГВС от ТН', 'cfg_dhw_max_from_compressor', 'temperature', '°C'),
            ('cfg_backup_type_name', 'Внешний нагреватель', 'cfg_backup_type_name', None, None),
            ('cfg_pump_relay_mode_name', 'Режим циркуляционного насоса', 'cfg_pump_relay_mode_name', None, None),
            ('cfg_flowmeter_type_name', 'Датчик протока', 'cfg_flowmeter_type_name', None, None),
            ('diag_sync_ms', 'Ping SYNC (мс)', 'diag_sync_ms', None, 'ms'),
            ('diag_cfg_ms', 'Ping CFG (мс)', 'diag_cfg_ms', None, 'ms'),
        ]

        for object_id, name, field, devcls, unit in sensors:
            cfg = {
                'name': name,
                'uniq_id': f'temzit_{object_id}',
                'stat_t': f'{MQTT_PREFIX}/state/{field}',
                'device': device,
                **avail,
            }
            if devcls:
                cfg['dev_cla'] = devcls
            if unit:
                cfg['unit_of_meas'] = unit
            self.publish(f'{MQTT_DISCOVERY_PREFIX}/sensor/temzit_{object_id}/config', cfg)

        for object_id, name, field in [
            ('dhw_heater_on', 'ТЭН БКН', 'dhw_heater_on'),
            ('compressor_active', 'Компрессор активен', 'compressor_active'),
        ]:
            self.publish(f'{MQTT_DISCOVERY_PREFIX}/binary_sensor/temzit_{object_id}/config', {
                'name': name,
                'uniq_id': f'temzit_{object_id}',
                'stat_t': f'{MQTT_PREFIX}/state/{field}',
                'payload_on': 'ON',
                'payload_off': 'OFF',
                'device': device,
                **avail,
            })

        self.publish(f'{MQTT_DISCOVERY_PREFIX}/sensor/temzit_alarm_text/config', {
            'name': 'Авария',
            'uniq_id': 'temzit_alarm_text',
            'stat_t': f'{MQTT_PREFIX}/state/alarm_text',
            'device': device,
            **avail,
        })

        self.discovery_sent = True

    def _publish_state(self, state: dict) -> None:
        mode_code = state.get('mode_code')
        state['ha_mode'] = MODE_CODE_TO_HA.get(mode_code, 'off')
        state['climate_target_temp'] = state.get('set_room')
        if state.get('compressor_active') is None:
            state['compressor_active'] = 'OFF'
        state['dhw_ha_mode'] = 'heat' if state['ha_mode'] != 'off' else 'off'

        sync_raw = state.get('_sync_raw')
        if sync_raw is not None:
            self.publish(f'{MQTT_PREFIX}/sync/raw', sync_raw, retain=False)

        self.publish(f'{MQTT_PREFIX}/state/json', state)
        for k, v in state.items():
            if v is not None and not k.startswith('_'):
                if isinstance(v, (dict, list)):
                    self.publish(f'{MQTT_PREFIX}/state/{k}', v)
                else:
                    self.publish(f'{MQTT_PREFIX}/state/{k}', str(v))

    def _publish_cfg(self, cfg: dict) -> None:
        self._last_cfg_raw = cfg.get('_raw')
        self.publish(f'{MQTT_PREFIX}/cfg/json', {k: v for k, v in cfg.items() if k != '_raw'})
        self.publish(f'{MQTT_PREFIX}/cfg/raw', cfg.get('_raw'))
        for k, v in cfg.items():
            if v is not None and k != '_raw':
                self.publish(f'{MQTT_PREFIX}/state/{k}', str(v))

    def maybe_poll_cfg(self) -> None:
        if TEMZIT_CFG_INTERVAL <= 0:
            return
        now = time.time()
        if now - self.last_cfg_poll < TEMZIT_CFG_INTERVAL:
            return
        wait = self.last_sync_ts + TEMZIT_CFG_DELAY_AFTER_SYNC - now
        if wait > 0:
            time.sleep(wait)
        try:
            cfg = self.temzit.get_cfg()
            self._publish_cfg(cfg)
            self.last_cfg_poll = time.time()
        except Exception as ce:
            print(f'CFG ERROR: {ce}', flush=True)
            self.publish(f'{MQTT_PREFIX}/bridge/error', {'cfg_error': str(ce)})

    def loop(self) -> None:
        self.client.will_set(f'{MQTT_PREFIX}/availability', 'offline', retain=True)
        self.client.connect(MQTT_HOST, MQTT_PORT, keepalive=60)
        self.client.loop_start()
        self.publish_discovery()
        while True:
            try:
                state = self.temzit.get_sync()
                self.last_sync_ts = time.time()
                self.publish(f'{MQTT_PREFIX}/availability', 'online')
                self._publish_state(state)
            except Exception as e:
                self.publish(f'{MQTT_PREFIX}/availability', 'degraded')
                self.publish(f'{MQTT_PREFIX}/bridge/error', {'sync_error': str(e)})
                time.sleep(TEMZIT_RETRY_DELAY)
                continue
            try:
                self.maybe_poll_cfg()
            except Exception as ce:
                self.publish(f'{MQTT_PREFIX}/bridge/error', {'cfg_error': str(ce)})
            time.sleep(TEMZIT_SYNC_INTERVAL)


if __name__ == '__main__':
    Bridge().loop()
